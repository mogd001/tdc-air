...
reporting_interval <- interval(start = ymd("20180101"), end = ymd("20230101"))
...
write_csv(site_meteorology_mapping, "outputs/site_meteorology_mapping.csv")
...
meteo <- get_meteorological_data(site_meteo, "20180101", "20230101")

write_csv(aqr_out, "outputs/meteorological_data_richmond_2018-2022.csv")

base_dates <- tibble(date = seq(min(meteo$date), max(meteo$date), by = "days"))
...
aqr_out <- aqr_day %>%
  select(
    "date", "value", "measurement", "site", "rainfall", "wind_kph_24h_avg", "wind_kph_max", "wind_kph_min", "wind_dir_24h_avg",
    "temp_degc_24h_avg", "temp_degc_max", "temp_degc_min"
  ) %>%
  filter(date >= ymd("20180101") & date < ymd("20230101"))

write_csv(aqr_out, "outputs/aq_data_richmond_2018-2022.csv")
...
aqm_pm2p5_day <- get_aq_data(site = site, measurement = "PM2.5 (24hr) [PM2.5 (24hr) TDC Partisol]") %>%
  mutate(measurement = "PM2.5", site = site)
aqm_pm10_day <- get_aq_data(site = site, measurement = "PM10 (24hr) [PM10 (24hr) TDC 2025i Partisol]") %>%
  mutate(measurement = "PM10", site = site)

# load rainfall and meteorology data
from <- "20180101" #format(min(aqm_day$date), "%Y%m%d")
to <- "20230101" #format(max(aqm_day$date), "%Y%m%d")
...
base_dates <- tibble(date = seq(min(meteo$date), max(meteo$date), by = "days"))
aqm_pm2p5_day <- base_dates %>%
  left_join(aqm_pm2p5_day, by = "date") %>%
  left_join(rainfall, by = "date") %>%
  left_join(meteo, by = "date") %>%
  mutate_at(vars(site), ~ replace_na(., !!site)) %>%
  mutate_at(vars(measurement), ~ replace_na(., "PM2.5"))
...
base_dates <- tibble(date = seq(min(meteo$date), max(meteo$date), by = "days"))
aqm_pm10_day <- base_dates %>%
  left_join(aqm_pm10_day, by = "date") %>%
  left_join(rainfall, by = "date") %>%
  left_join(meteo, by = "date") %>%
  mutate_at(vars(site), ~ replace_na(., !!site)) %>%
  mutate_at(vars(measurement), ~ replace_na(., "PM10"))

aqm_day <- bind_rows(aqm_pm2p5_day, aqm_pm10_day)
...
aqm_out <- aqm_day %>%
  ...
  filter(date >= ymd("20180801") & date < ymd("20230101"))

write_csv(aqm_out, "outputs/aq_data_motueka_goodman_park_2018-2022.csv")
