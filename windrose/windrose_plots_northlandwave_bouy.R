...
f1 <- "data/wave04_05_23_19_22_53.csv"

northland_wave_bouy <- read_csv(f1) %>% 
  transmute(speed = sig_wave_height_m, direction = mean_wave_direction, datetime = dmy_hm(datetime_nzst, tz = "Etc/GMT-12"))

site <- "northland_wave_bouy"
df <- northland_wave_bouy
...
summary %>% 
  ...
  write_csv(glue("outputs/windrose_summary_northlandbouy.csv"))