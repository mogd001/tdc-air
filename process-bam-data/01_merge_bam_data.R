...
p1 <- ggplot(long_5min, aes(datetime, pm10)) +
  ...
  labs(x = "", y = "PM10 (ug/m3)", title = "PM10 Raw (5 minute)") + 
...
p2 <- ggplot(long_5min, aes(datetime, pm2p5)) +
  ...
  labs(x = "", y = "PM2.5 (ug/m3)", title = "PM2.5 Raw (5 minute)") + 
...
p_long_daily_raw <- ggplot(long_daily)  +
  geom_step(mapping = aes(datetime, pm10_daily, color = "PM10"), size = 0.5) +
  geom_step(mapping = aes(datetime, pm2p5_daily, color = "PM2.5"), size = 0.5) +
  labs(x = "", y = "PM (ug/m3)" , title = "PM Daily Raw") + 
